def shuffle(s):
    ...
    "*** YOUR CODE HERE ***"
    s, mid = list(s), len(s) // 2
    l, r = s[:mid], s[mid:]
    return [x for pair in zip(l, r) for x in pair]
...
def balanced(m):
    ...
    if is_planet(m):
        ...
        res = balanced(end(left(m))) and balanced(end(right(m)))

        l_mass = total_mass(end(left(m)))
        l_torque = length(left(m)) * l_mass
        r_mass = total_mass(end(right(m)))
        r_torque = length(right(m)) * r_mass

        res = res and l_torque == r_torque
        return res
